### -- ---------------------------------------------------------------------------------------------------------
### -- Script Name		: PRINT DB Artifacts deploy
### -- Deploy Module 	: MOSIP PRINT
### -- Purpose    		: To deploy MOSIP PRINT Database DB Artifacts.
### -- Create By   		: Thamaraikannan C
...
#========================================DB Deployment process begins on PRINT DB SERVER======================================
...
MASTERCONN=$(PGPASSWORD=$SU_USER_PWD  psql --username=$SU_USER --host=$DB_SERVERIP --port=$DB_PORT --dbname=$DEFAULT_DB_NAME -t -c "select count(1) from pg_roles where rolname IN('mspprintuser')";exit; >> $LOG 2>&1)

if [ ${MASTERCONN} == 0 ]
then
    echo `date "+%m/%d/%Y %H:%M:%S"` ": Creating print database user" | tee -a $LOG 2>&1
    ...
    echo `date "+%m/%d/%Y %H:%M:%S"` ": print database user already exist" | tee -a $LOG 2>&1
fi
...
#========================================DB Deployment process completes on PRINT DB SERVER======================================
